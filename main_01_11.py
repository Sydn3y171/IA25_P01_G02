...
def try_solve_with_budget(data, total_seconds=90.0):
    """
...
# ---- UC sem nome ou turma ----
def validate_dataset(data):
    UCs = data["UCs"]
    miss_t = [uc for uc in UCs if uc not in data["uc_to_teacher"]]
    miss_c = [uc for uc in UCs if uc not in data["uc_to_class"]]
    if miss_t or miss_c:
        msgs = []
        if miss_t: msgs.append(f"UCs sem docente: {', '.join(miss_t)}")
        if miss_c: msgs.append(f"UCs sem turma: {', '.join(miss_c)}")
        raise ValueError("Dataset inválido: " + " | ".join(msgs))
